...
exports.register = async (req, res) => {
  ...
  if (!name || !email || !password || !passwordConfirm) {
    return res.redirect(
      '/register?message=' + encodeURIComponent('Please fill in all fields!')
    )
  }
...
exports.resendVerificationEmail = async (req, res) => {
  ...
  db.query('SELECT * FROM users WHERE email = ?', [email], (error, results) => {
    ...
    db.query(
      'UPDATE users SET activation_token = ?, reset_token_expires = ? WHERE email = ?',
      [activationToken, expiresAt, email],
      (err, updateResult) => {
        if (err) {
          console.log(err)
          return res
            .status(500)
            .render('login', { message: 'Failed to send verification email' })
        }
  ...
  db.query(
    ...
    async (error, results) => {
      ...
      const expiresAt = moment()
        .add(60, 'minutes')
        .format('YYYY-MM-DD HH:mm:ss') // Set 30 menit dari sekarang
        ...
        (err) => {
          ...
          transporter.sendMail(mailOptions, (mailErr, info) => {
            ...
            res.render('forgot-password', {
              message2:
                'Verification link sent successfully, Please check your inbox and enter new password'
            })
          })
        }
      )
...
exports.resetPassword = async (req, res) => {
  ...
  if (!code || code.length !== 5) {
    return res.render('reset-password', {
      message:
        'Invalid or incomplete verification code. Try to fill in the code without using copy-paste.'
    })
  }
