...
const validateToken = async (req, res, next) => {
    ...
    try {
        ...
        const foundToken = await Token.findOne({ where: { activation_token: token } });
        ...
        if (currentTime.isAfter(expirationTime)) {
            ...
            return res.status(403).render('login', { message: 'Code expired. Please request a new one.' }); // Render halaman login dengan pesan token expired
        }
        ...
        return res.status(500).render('login', { message: 'Server error, please try again later.' }); // Handle error dan render halaman code-verification dengan pesan error
    }
};
